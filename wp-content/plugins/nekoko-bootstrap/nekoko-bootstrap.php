<?php
/**
 * Plugin Name:       NekoKo Bootstrap
 * Description:       One-time site setup: activates required plugins, sets theme, permalinks, and SMTP config. Self-guards via option flags so it only runs once. Converted from a must-use plugin to a regular plugin.
 * Version:           1.0.0
 * Author:            NekoKo
 * Text Domain:       nekoko-bootstrap
 */
...
function nekoko_bootstrap_run() {
	...
	update_option( 'blogdescription', 'Platforma za nišne usluge u Srbiji' );
...
function nekoko_configure_smtp() {
	if ( get_option( 'nekoko_smtp_configured' ) ) {
		return;
	}
